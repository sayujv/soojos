...
**Since 25 September a launch agent runs `desk_beat.py` every 5 minutes** (gate, tick, inbox
sync with Haiku routing, dispatch of `auto_dispatch` tasks, board). Your half-hour heartbeat
therefore finds routine work already done; what remains for you is coordinator work: tasks
not marked for automatic dispatch (your review task is one), judgement calls, digests, and
the handoff. `~/.soojos/desk/beat.log` shows what each beat did.

**Step 0 of every beat, before any model turn:**

```sh
/usr/bin/python3 /Users/sayuj/soojos/tools/soojos-desk/heartbeat_gate.py
```

`UNCHANGED` (exit 0): end the beat; write nothing. `ATTENTION: …` (exit 10): the reasons name
what to look at. Decision 0007 explains why: 128 of your 158 entries from 19 to 24 September
were unchanged checkpoints, and that is where the credits went.
